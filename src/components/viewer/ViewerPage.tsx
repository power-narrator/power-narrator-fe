...
export function ViewerPage({ slides: initialSlides, filePath, onBack }: ViewerPageProps) {
    ...
    const confirmSync = () => window.confirm('Syncing will override any unsaved changes in your notes. Do you want to proceed?');
    ...
    const syncSlides = async (request: Promise<SlidesElectronResult>, failureMessage: string) => {
        ...
        try {
            ...
            setActiveSlideIndex((currentIndex) => Math.min(currentIndex, Math.max(0, result.slides!.length - 1)));
        } catch (error: unknown) {
    ...
    const handleSyncSlide = async () => {
        ...
        await syncSlides(
            electronAPI.syncSlide({ filePath, slideIndex: getSlideNumber(activeSlide, activeSlideIndex) }),
            'Sync slide error',
        );
    };
    ...
    if (!electronAPI) {
        return (
            <div style={{ height: '100vh', width: '100vw', display: 'flex', flexDirection: 'column' }}>
                ...
                <Box p="xl">
                    <Text c="red" fw={600} mb="sm">Electron preload API is unavailable.</Text>
