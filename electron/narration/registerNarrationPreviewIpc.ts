...
export function registerNarrationPreviewIpc(
  ...
  ipc.handle(
    "prepare-narration-preview",
    async (_, request: PreviewNarrationRequest): Promise<NarrationPreviewResult> => {
      const { audio, mediaType } = await narrationPreparation.preparePreview(request);
      return { audio: new Uint8Array(audio), mediaType };
    },
  );
}
